      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className="fixed top-0 right-0 h-full w-full max-w-md bg-[#0a0a0a] z-[70] flex flex-col font-sans text-white border-l border-white/5 shadow-2xl"
          >
            {/* Header */}
                    ...
                    <div className="grid grid-cols-[80px_1fr] gap-0">
                      ...
                      <div className="p-3 flex flex-col justify-between">
                        <div className="flex justify-between items-start">
                          <div className="flex-1 min-w-0 pr-2">
                            ...
                            <div className="flex items-center gap-3 mt-1">
                              {item.size && (
                                <span className="text-[9px] font-mono bg-nivis-neon/10 text-nivis-neon px-2 py-0.5 rounded uppercase tracking-wider border border-nivis-neon/20">
                                  Talle {item.size}
                                </span>
                              )}
                              <span className="text-[9px] font-mono text-white/30 uppercase">{item.category || 'Producto'}</span>
                            </div>
                          </div>
                          <button 
                        ...
                        <div className="flex justify-between items-center mt-2 pt-2 border-t border-white/5">
                          {/* Quantity Controls */}
                          <div className="flex items-center gap-0 border border-white/10 rounded-sm">
                            <button 
                              onClick={() => updateQuantity(item.id, -1, item.size)} 
                              className="px-2.5 py-1 text-white/40 hover:text-white text-xs font-bold transition-colors"
                            >−</button>
                            <span className="px-3 py-1 text-xs font-black border-x border-white/10">{item.quantity}</span>
                            <button 
                              onClick={() => updateQuantity(item.id, 1, item.size)} 
                              className="px-2.5 py-1 text-white/40 hover:text-white text-xs font-bold transition-colors"
                            >+</button>
                          ...
                          <div className="text-right">
                            ...
                            {item.quantity > 1 && (
                              <p className="text-[9px] text-white/30 font-mono">${item.price.toLocaleString()} c/u</p>
                            )}
                          </div>
                        </div>
                      </div>
